...
export const DEFAULT_FEMALE_SIZES = ["S", "M", "L", "XL"];
export const DEFAULT_MALE_SIZES = ["M", "L", "XL"];

export function isMenProduct(product) {
  if (!product) return false;
  const category = String(product.category || "").toLowerCase();
  if (category.includes("women")) return false;
  if (category.includes("men")) return true;
  const subCategory = String(product.subCategory || "").toLowerCase();
  if (["sherwani", "bandhgala", "waistcoat"].some((s) => subCategory.includes(s))) return true;
  return false;
}
...
function TabSection({ product }) {
  ...
  return (
    <div className={styles.tabs} id="tabs-section">
      ...
      <div className={styles.tabContent} role="tabpanel">
        {tab === "description" && (
          <div className={styles.descContent}>
            ...
            <ul className={styles.descList}>
              ...
              {product.craft && <li>Craft: {product.craft}</li>}
              {product.noOfComponents && <li>No. of Components: {product.noOfComponents}</li>}
        ...
        {tab === "info" && (
          <div className={styles.infoContent}>
            <h3 className={`display ${styles.infoTitle}`}>Size Guide ({isMen ? "Men's Sizing" : "Women's Sizing"})</h3>
    ...
    <div className={styles.related}>
      ...
      <div className={styles.relatedGrid}>
        {products.map((p) => (
          <a
            key={p.id}
            href={`/product/${(p.slug || p.id).toLowerCase()}`}
...
export default function ProductContent({ product, related }) {
  ...
  const isMen = isMenProduct(product);
  const defaultSizes = isMen ? DEFAULT_MALE_SIZES : DEFAULT_FEMALE_SIZES;
        ...
        <div className={styles.divider} />
          ...
          {product.disclaimer && (
            <div className={styles.disclaimerBox}>
              ...
              <p className={styles.disclaimerText}>{product.disclaimer}</p>
            </div>
          )}
          ...
          <dl className={styles.productMeta}>
            ...
            {product.color && <div className={styles.metaRow}><dt>Color</dt><dd>{product.color}</dd></div>}
            <div className={styles.metaRow}><dt>Fabric</dt><dd>{product.fabric}</dd></div>
            {product.craft && <div className={styles.metaRow}><dt>Craft</dt><dd>{product.craft}</dd></div>}
            {product.noOfComponents && <div className={styles.metaRow}><dt>No. of Components</dt><dd>{product.noOfComponents}</dd></div>}
